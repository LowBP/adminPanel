...
class ViewSkusRowExpand extends Component {
    constructor(props) {
        super(props);
        this.state = {
            sku_name: '',
    ...
    initializeSku(data) {
        ...
        var store_t_sku_data = {
            name: data.sku_expand_data.sku_name,
            manufacture_name: data.sku_expand_data.manu_name,

        };

        console.log(store_t_sku_data)
    ...
    tableInputNameField(cellValue, rowData, nothing, cellFeild) {
        return (
            <input value={cellValue} readOnly required className='filter text-filter form-control readonly-bg   ' onChange={(data) => this.setState({ sku_name: data.target.value })} onBlur={(data) => rowData[cellFeild] = this.setCodeFromat(data.target.value)} />
        );
    }
    setCodeFromat(code) {
        var code_value = "";
    ...
    onChangeManufactureUpdate(rowData, event) {
        ...
        this.setState({ manu_value: event });
        this.setState({ skus });
    }
    tableInputManufactureField(cellValue, rowData, nothing, cellFeild) {
        return <input value={cellValue} readOnly required className='filter text-filter form-control readonly-bg   ' onChange={(data) => this.setState({ sku_name: data.target.value })} onBlur={(data) => rowData[cellFeild] = this.setCodeFromat(data.target.value)} />

        // return (
        //     <Select
        //         value={this.state.manu_value}
...
ViewSkusRowExpand.propTypes = {

};

export default ViewSkusRowExpand;