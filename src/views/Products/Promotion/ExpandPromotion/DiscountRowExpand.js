...
class DiscountRowExpand extends Component {
    constructor(props) {
        super(props);
        this.state = {
            promotions: {
            ...
            offeramount: this.props.data.offeramount,
    ...
    tableInputOfferAmountField(cellValue, rowData, nothing, cellFeild) {


        var class_import;
        if (rowData.offeramount != "" && rowData.offeramount > 0) {
            rowData.offeramount_found = true;
            class_import = 'filter text-filter form-control valid-border-clr';

        }
        else {
            rowData.offeramount_found = false;
        ...
        if (rowData.add_new != true)
            ...
            return <input type="text" value={this.state.offeramount} required pattern="[0.00 - 9.00]*" className={class_import} onChange={this.checkOfferNumberValidity.bind(this)} onBlur={(data) => (rowData[cellFeild] = this.state.offeramount, this.props.onChangeValue(rowData))} />
    ...
    tableInputMinField(cellValue, rowData, nothing, cellFeild) {
        ...
        if (rowData.add_new != true)
            ...
            return <input type="text" step="any" min="1" value={this.state.min} pattern="[0.00 - 9.00]*" onChange={this.checkMinValidation.bind(this)} className={class_import} required onBlur={(data) => (rowData[cellFeild] = this.state.min, this.props.onChangeValue(rowData))} />
    ...
    tableInputMaxField(cellValue, rowData, nothing, cellFeild) {
        ...
        if (rowData.add_new != true)
            ...
            return <input type="text" min={1} value={this.state.max} onChange={this.checkMaxValidation.bind(this)} required className={class_import} onBlur={(data) => (rowData[cellFeild] = this.state.max, this.props.onChangeValue(rowData))} />
    ...
    tableInputPriorityDateField(cellValue, rowData, nothing, cellFeild) {
        ...
        // else
        return <input type="text" min="1" value={this.state.priority} required className={class_import} onChange={this.checkPriorityValidation.bind(this)} onBlur={(data) => (rowData[cellFeild] = this.state.priority, this.props.onChangeValue(rowData))} />
    ...
    tableInputCustomerLimitField(cellValue, rowData, nothing, cellFeild) {
        ...
        //     return <input type="number" step="any" min="0" value={cellValue} required readOnly className='filter text-filter form-control readonly-bg  ' onBlur={(data) => rowData[cellFeild] = data.target.value} />
        // else
        return <input type="text" step="any" min="0" value={this.state.customerLimit} onChange={this.checkCustomerLimitValidation.bind(this)} required className={class_import} onBlur={(data) => (rowData[cellFeild] = this.state.customerLimit, this.props.onChangeValue(rowData))} />
                    ...
                    <div class="col-lg-11">
                        ...
                        <div style={{ 'margin-left': '80px' }}>
                            ...
                            >
                                ...
                                <TableHeaderColumn dataAlign='center' dataField='offeramount' width="210" expandable={false} isKey={true} expandable={false} dataFormat={this.tableInputOfferAmountField.bind(this)} >Offer Amount </TableHeaderColumn>
...
DiscountRowExpand.propTypes = {

};

export default DiscountRowExpand;