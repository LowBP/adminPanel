...
const expiry_time_limit = 3500000;
...
export default {
  ...
  setToken() {
    ...
    var startTime = reactLocalStorage.get('startTime');
    reactLocalStorage.set('currentTime', currentTime);
    var compareTime = currentTime - startTime;

    console.log("compareTime", compareTime);
    if (compareTime > expiry_time_limit || compareTime < 0) {

      console.log("tokenexpired");

      reactLocalStorage.set('startTime', currentTime);
      ...
      if (firebase.auth().currentUser != null) {
        ...
        firebase.auth().onAuthStateChanged(function (user) {
          if (user) {
            ...
            firebase.auth().currentUser.getIdToken(true).then(function (idToken) {
              reactLocalStorage.set('token', idToken);
              console.log("refresh token token", idToken);
